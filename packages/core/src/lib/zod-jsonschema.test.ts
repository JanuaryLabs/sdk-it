...
describe('evalZod', () => {
  ...
  test('preserves .describe() through ZodOptional override for all coerce types with .nullish()', async () => {
    const cases = [
      {
        name: 'coerce-number',
        source: 'z.coerce.number().nullish().describe("num desc")',
        desc: 'num desc',
      },
      {
        name: 'coerce-string',
        source: 'z.coerce.string().nullish().describe("str desc")',
        desc: 'str desc',
      },
      {
        name: 'coerce-boolean',
        source: 'z.coerce.boolean().nullish().describe("bool desc")',
        desc: 'bool desc',
      },
      {
        name: 'coerce-bigint',
        source: 'z.coerce.bigint().nullish().describe("bigint desc")',
        desc: 'bigint desc',
      },
      {
        name: 'coerce-date',
        source: 'z.coerce.date().nullish().describe("date desc")',
        desc: 'date desc',
      },
    ] as const;
  ...
  test('preserves .describe() on required schemas (no optional/nullable wrapper)', async () => {
    const cases = [
      {
        name: 'z.date()',
        source: 'z.date().describe("a date")',
        desc: 'a date',
        type: 'string',
        extra: { format: 'date-time', 'x-zod-type': 'date' },
      },
      {
        name: 'z.coerce.date()',
        source: 'z.coerce.date().describe("coerced date")',
        desc: 'coerced date',
        type: 'string',
        extra: { format: 'date-time', 'x-zod-type': 'coerce-date' },
      },
      {
        name: 'z.string()',
        source: 'z.string().describe("a string")',
        desc: 'a string',
        type: 'string',
        extra: {},
      },
      {
        name: 'z.number()',
        source: 'z.number().describe("a number")',
        desc: 'a number',
        type: 'number',
        extra: {},
      },
      {
        name: 'z.boolean()',
        source: 'z.boolean().describe("a bool")',
        desc: 'a bool',
        type: 'boolean',
        extra: {},
      },
  ...
  test('preserves .describe() on compound types with .nullish()', async () => {
    const cases = [
      {
        name: 'object',
        source: 'z.object({ a: z.string() }).nullish().describe("obj desc")',
        desc: 'obj desc',
      },
      {
        name: 'array',
        source: 'z.array(z.string()).nullish().describe("arr desc")',
        desc: 'arr desc',
      },
      {
        name: 'enum',
        source: 'z.enum(["a","b"]).nullish().describe("enum desc")',
        desc: 'enum desc',
      },
    ] as const;
